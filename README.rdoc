...
Dupe does not support post,put,delete verbs.  Using dupe will cause any post,put,delete to fail with an unclear exception message.  Dupe is awesome for get requests, but if you will ever need to support the others verbs, this is probably not the right tool for the job yet.
...
===POST requests (not currently implemented)
...
===PUT requests (not currently implemented)
...
===DELETE requests (not currently implemented)

As you might have guessed, Dupe also doesn't support DELETE intercept mocking (ActiveResource::Base#destroy):
