...
Using Dupe, they were able to cuke new frontend, mocking
...
services they needed to expose on the backend, and the format of the XML responses. Once those services existed, data migration (over the services) was a breeze, and the new
site had a nicely decoupled, service-oriented architecture. 
...
This would send off a request like "/books.xml?author_id=1". If this happened during the context of a cucumber scenario, then Dupe would throw an exception with a message like:
  ...
  There is no custom service mapping for "/books.xml?author_id=1". Now go to features/support/custom_mocks.rb and add it. (StandardError)

If the file features/support/custom_mocks.rb does not yet exist, create it and paste this into it: 
...
Dupe will take the result (a Hash), and convert it to XML. You could have alternatively given a string of XML (or anything), and Dupe would have taken it as-is. 
