...
describe Dupe::Mock do
  describe "new" do
    it "should require a valid REST type" do
      proc { Dupe::Mock.new :unknown, /\//, proc {} }.should raise_error(
        ArgumentError,
        "Unknown REST verb ':unknown'. Valid REST verbs are :get, :post, :put, and :delete."
      )
      proc { Dupe::Mock.new :get,     /\//, proc {} }.should_not raise_error
      proc { Dupe::Mock.new :post,    /\//, proc {} }.should_not raise_error
      proc { Dupe::Mock.new :put,     /\//, proc {} }.should_not raise_error
      proc { Dupe::Mock.new :delete,  /\//, proc {} }.should_not raise_error
    end
    
    it "should require the url be a kind of regular expression" do
      proc { Dupe::Mock.new :get, '', proc {} }.should raise_error(
        ArgumentError,
    ...
    it "should set the @verb, @url, and @response parameters accordingly" do
      url_pattern = /\//
      response = proc {}
      mock = Dupe::Mock.new :get, url_pattern, response
  ...
  describe "match?" do
    it "should determine if a given string matches the mock's url pattern" do
      url = %r{/blogs/(\d+).xml}
      response = proc {}
      mock = Dupe::Mock.new :get, url, response
